...
class ABC_grid_hdf5_sdf(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            _, gt_output_float_,gt_input_ = read_and_augment_data_ndc(self.data_dir, self.file_names[index], grid_size, self.train,
                                                                   aug_permutation=True,aug_reversal=True,aug_inversion=True)
        else:
            if self.input_only:
                _, gt_output_float_,gt_input_ = read_data_input_only(self.data_dir, self.file_names[index], grid_size)
            else:
                _, gt_output_float_,gt_input_ = read_data(self.data_dir, self.file_names[index], grid_size)
        ...
        # clip to ignore far-away cells
        gt_input = np.clip(gt_input, -2, 2)
        gt_input = resize(gt_input, (1, 64, 64, 64))
